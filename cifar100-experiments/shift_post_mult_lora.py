...
class PostMult_LoRALayer(nn.Module):
    ...
    def forward(self, x):
        if isinstance(self.base_layer, nn.Linear):
            # Compute BA
            BA = self.shift(torch.matmul(self.A, self.B))  # Shape: (in_features, in_features)
            ...
            adapted_weight = (weight_reshaped.T @ self.shift(self.B @ self.A)).view(self.base_layer.weight.shape)
            ...
            raise ValueError("Shift_PostMult_LoRALayer only supports nn.Linear and nn.Conv2d layers")

def apply_post_mult_lora(model, rank=4):
    """
    Recursively applies Shift_PostMult_LoRALayer to all nn.Linear and nn.Conv2d layers in the model.
    """
...
if __name__ == "__main__":
    ...
    print(f"Model fine-tuned using modified Shift Post-Mult LoRA on classes: {classes}")

    ranks = [1, 2, 4, 8, 16, 32, 64]
    histories = {}

    for rank in ranks:
        print(f"\nApplying modified Shift Post-Multiplication LoRA for rank = {rank}")
        ...
        # Save the model
        model_save_path = f"./out_files/resnet50_Shift_Post_Mult_LoRA_model_rank_{rank}.pt"
        torch.save(model.state_dict(), model_save_path)
        print(f"Model saved to {model_save_path}")

    # Save all histories
    histories_save_path = "./out_files/resnet50_Shift_Post_Mult_LoRA_model_histories.pkl"
