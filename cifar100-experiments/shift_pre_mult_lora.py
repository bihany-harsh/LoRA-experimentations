...
class Shift_PreMult_LoRALayer(nn.Module):
    def __init__(self, base_layer, rank=4):
        super(Shift_PreMult_LoRALayer, self).__init__()
    ...
    def forward(self, x):
        if isinstance(self.base_layer, nn.Linear):
            # Multiply BAW
            return F.linear(x,
                            self.shift(self.B @ self.A) @ self.base_layer.weight, 
            ...
            adapted_weight = (self.shift(self.B @ self.A) @ weight_reshaped).view(self.base_layer.weight.shape)
            ...
            raise ValueError("Shift_PreMult_LoRALayer only supports nn.Linear and nn.Conv2d layers")

def apply_pre_mult_lora(model, rank=4):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear) or isinstance(module, nn.Conv2d):
            setattr(model, name, Shift_PreMult_LoRALayer(module, rank=rank))
        else:
...
if __name__ == "__main__":
    ...
    print(f"Model finetuned using modified Shift Pre Mult LoRA on: {classes}")
    ...
    for rank in ranks:
        ...
        torch.save(model.state_dict(), f"./out_files/resnet50_Shift_Pre_Mult_LoRA_model_rank_{rank}.pt")
        print(f"Model saved to ./out_files/resnet50_Shift_Pre_Mult_LoRA_model_rank_{rank}.pt")

    with open("./out_files/resnet50_Shift_Pre_Mult_LoRA_model_histories.pkl", "wb") as f:
        pickle.dump(histories, f)

    print("Histories saved to ./out_files/resnet50_Shift_Pre_Mult_LoRA_model_histories.pkl")
