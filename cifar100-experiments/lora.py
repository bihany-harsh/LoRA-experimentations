...
class LoRALayer(nn.Module):
    def __init__(self, base_layer, rank=4):
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            self.lora_B = nn.Parameter(
                base_layer.weight.new_zeros(( in_features, self.rank ))
            )
    ...
    def forward(self, x):
        if isinstance(self.base_layer, nn.Linear):
            adapted_weight = self.base_layer.weight + torch.matmul(self.lora_B, self.lora_A).T
            ...
            return self.base_layer._conv_forward(
                x,
                self.base_layer.weight + (self.lora_B @ self.lora_A).view(self.base_layer.weight.shape),
...
if __name__ == "__main__":
    ...
    print(f"Model finetuned using LoRAS on: {classes}")
    ...
    for rank in ranks:
        ...
        torch.save(model.state_dict(), f"./out_files/resnet50_LoRA_model_rank_{rank}.pt")
        print(f"Model saved to ./out_files/resnet50_LoRA_model_rank_{rank}.pt")

    with open("./out_files/resnet50_LoRA_model_histories.pkl", "wb") as f:
        pickle.dump(histories, f)

    print("Histories saved to ./out_files/resnet50_LoRA_model_histories.pkl")
