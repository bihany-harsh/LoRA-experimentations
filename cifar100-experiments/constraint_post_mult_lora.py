...
class Constraint_PostMult_LoRALayer(nn.Module):
    def __init__(self, base_layer, rank=4):
        super(Constraint_PostMult_LoRALayer, self).__init__()
        ...
        if isinstance(base_layer, nn.Linear):
            ...
            self.B_1 = nn.Parameter(
                torch.zeros(in_channels, self.rank * kernel_size * kernel_size, device=base_layer.weight.device)
            )
            self.A_1 = nn.Parameter(
                torch.zeros(self.rank * kernel_size * kernel_size, in_channels, device=base_layer.weight.device)
            )
    ...
    def reset_parameters(self):
        in_features = self.B_1.size(0)
        if self.kernel_size is not None:
            combined_B = nn.init.orthogonal_(torch.empty((in_features, 2 * self.rank * self.kernel_size * self.kernel_size), device=self.B_1.device, dtype=self.B_1.dtype))
        else:
            combined_B = nn.init.orthogonal_(torch.empty((in_features, 2*self.rank), device=self.B_1.device, dtype=self.B_1.dtype))
    ...
    def forward(self, x):
        if isinstance(self.base_layer, nn.Linear):
            # (W @ B_1) @ A_1 + (W @ B_1) @ A_1
            adapted_weight = (self.base_layer.weight @ self.B_1) @ self.A_1 + (self.base_layer.weight @ self.B_2) @ self.A_2

            return F.linear(x, adapted_weight, self.base_layer.bias)
        elif isinstance(self.base_layer, nn.Conv2d):
            weight_reshaped = self.base_layer.weight.view(self.base_layer.weight.size(1), -1)
            adapted_weight = ((weight_reshaped.T @ self.B_1) @ self.A_1 + (weight_reshaped.T @ self.B_2) @ self.A_2).view(self.base_layer.weight.shape)
...
def apply_constraint_post_mult_lora(model, rank=4):
    for name, module in model.named_children():
        if isinstance(module, nn.Linear) or isinstance(module, nn.Conv2d):
            setattr(model, name, Constraint_PostMult_LoRALayer(module, rank=rank))
        else:
            apply_constraint_post_mult_lora(module, rank=rank)
        ...
        with tqdm(total=len(train_dataloader), desc=f"Epoch {epoch + 1}/{epochs}", unit="batch", leave=False) as pbar:
            for i, (inputs, labels) in enumerate(train_dataloader):
                ...
                for module in model.modules():
                    if isinstance(module, Constraint_PostMult_LoRALayer):
...
if __name__ == "__main__":
    ...
    for rank in ranks:
        print(f"\nApplying modified Constraint Post-Multiplication LoRA for rank = {rank}")
        ...
        apply_constraint_post_mult_lora(model, rank=rank)  # Use the new function for constraint LoRA
        model.to(device)

        trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
        total_params = sum(p.numel() for p in model.parameters())
        print(f"For Constraint Post Mult LoRA rank: {rank}, #trainable_params: {trainable_params} and #total_params: {total_params}")
        ...
        # Save the model
        model_save_path = f"./out_files/resnet50_Constraint_PostMult_LoRA_model_rank_{rank}.pt"
        torch.save(model.state_dict(), model_save_path)
        print(f"Model saved to {model_save_path}")

    # Save all histories
    histories_save_path = "./out_files/resnet50_Constraint_PostMult_LoRA_model_histories.pkl"
