...
class MNIST_FFN_Pre_Mult_LoRA(nn.Module):
    def __init__(self, hidden_size, num_layers, classes):
        ...
        # Mult-LoRA params
        self.lin_mult_lora_A = nn.Parameter(torch.eye(channels*width*height))
        self.mult_lora_As = nn.ParameterList()
        for i in range(self.num_layers):
            self.mult_lora_As.append(nn.Parameter(torch.eye(self.hidden_size)))
        
        self.lout_mult_lora_A = nn.Parameter(torch.eye(self.hidden_size))
...
def pre_mult_lora_experiment(
    ...
    model = MNIST_FFN_Pre_Mult_LoRA(hidden_size, num_layers, original_classes)
    ...
    print("Pre-Mult-LoRA full rank experiment")
    ...
    with open(f"./out_files/MNIST_{model.hidden_size}_{model.num_layers}_pre_mult_lora_FFN_history.pkl", "wb") as f:
        pickle.dump(history, f)

    torch.save(model.state_dict(), f"./out_files/MNIST_{model.hidden_size}_{model.num_layers}_pre_mult_lora_FFN.pt")
