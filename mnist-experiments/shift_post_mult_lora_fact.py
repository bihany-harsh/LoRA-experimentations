...
def post_mult_lora_experiment(
    ...
    print("Post-Mult-LoRA experiment: lora_rank =", rank)
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total_params = sum(p.numel() for p in model.parameters())

    print(f"For Post-Mult-LoRA rank: {rank}, #trainable_params: {trainable_params} and #total_params: {total_params}")
    ...
    with open(f"./out_files/post_mult_lora/MNIST_{model.hidden_size}_{model.num_layers}_rank_{rank}_post_mult_lora_FFN_history.pkl", "wb") as f:
        pickle.dump(history, f)

    torch.save(model.state_dict(), f"./out_files/post_mult_lora/MNIST_{model.hidden_size}_{model.num_layers}_rank_{rank}_post_mult_lora_FFN.pt")
